...
test_that("convert to numeric", {
    ...
    create.test.data <- function(hic_code, val) {
        ...
        rep.data <- as.data.frame(t(rep(val, length(stname))))
    ...
    expect_equal(as.POSIXct("2010-01-01"), 
                 as.POSIXct(as.numeric(result[1]), origin="1970-01-01"))
    ...
    test_data <- create.test.data(tdatetime, "2010-01-01 00:00:00")
    result <- convert.numeric.datetime(test_data)
    expect_equal(length(unique(t(result))), 1)
    expect_true(!NA %in% result)
})
